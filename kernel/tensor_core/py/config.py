...
parser.add_argument('--num_thread', '-t', type=int, default=4,help='Number of threads in system')
parser.add_argument('--num_warps', '-w', type=int, default=4, help='Number of warps')
parser.add_argument('--num_cores', '-c', type=int, default=2, help='Number of threads in system')
...
with open(args["define_file"], "w") as f:
    f.write(f"-DM={args['M']} -DN={args['N']} -DK={args['K']} -DTC_OP_SIZE={getCSize(args['op_type'])} -DTC_RES_SIZE={getCSize(args['res_type'])} {system.getdef()} {tc.getdef()} ")
    f.close()
