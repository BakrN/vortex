...
# Cycles and timings
header = "exp_name,threads,dot_units,ibuf_stalls,scrb_stalls,ipc,instrs,cycles"
with open("combined_info.csv", "w") as f :
    ...
    for exp in dirs:
        ...
        f.write(f"{info.name()},{info.threads},{info.mac_units},{info.ibuf_stalls},{info.scrb_stalls},{info.IPC},{info.instrs},{info.cycles}")
        for stat_name in df['stat_name'].unique():
