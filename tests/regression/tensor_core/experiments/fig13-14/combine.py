...
# Cycles and timings
header = "wg_size,num_regs,ibuf_stalls,scrb_stalls,ipc,instrs,cycles"
with open("combined_info.csv", "w") as f :
    ...
    for exp in dirs:
        print(exp)
        reg, wg = exp.split("reg")[1].split("wg")# .split("
        info = ExpInfo.from_file(f"{exp}/out.log")
        # Run to get median and mean here
        csv_file_path = f"{exp}/statistic.txt"
        df = pd.read_csv(csv_file_path)
        
        f.write(f"{reg},{wg},{info.ibuf_stalls},{info.scrb_stalls},{info.IPC},{info.instrs},{info.cycles}")
        for stat_name in df['stat_name'].unique():
