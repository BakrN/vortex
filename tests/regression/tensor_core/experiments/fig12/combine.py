...
# Cycles and timings
header = "exp_name,threads,dot_units,num_regs,ibuf_stalls,scrb_stalls,ipc,instrs,cycles"
with open("combined_info.csv", "w") as f :
    ...
    for exp in dirs:
        ...
        info.num_regs = int(regs)
        # Run to get median and mean here
        csv_file_path = f"{exp}/statistic.txt"
        df = pd.read_csv(csv_file_path)
        
        f.write(f"{info.name()},{info.threads},{info.mac_units},{info.num_regs},{info.ibuf_stalls},{info.scrb_stalls},{info.IPC},{info.instrs},{info.cycles}")
        for stat_name in df['stat_name'].unique():
