...
template <typename T,
    ...
    inline void tc_store( T* regD , T* res_addr, int thread_id, const int MAT_M , const int MAT_N)  {
        ...
        if constexpr (layout == layout_t::ROW_MAJOR) {
            ...
            unrolled_store_row_row_major<0, 0,NUM_ROWS,TILE_COLS,T, NUM_PE_PER_GROUP>(c_row_ptr, regD, MAT_N);
        } else {
            ...
            T* c_row_ptr = res_addr + c_row_offset + c_col_offset;
            unrolled_store_row_col_major<0, 0,NUM_ROWS,TILE_COLS,T, NUM_PE_PER_GROUP>(c_row_ptr, regD, MAT_M);
        }
}
...
inline void tc_flush_tiles(T* dest) {// flush
    ...
    if constexpr(CUR_ROW_TILE < NUM_ROW_TILES-1) {
        if constexpr (CUR_COL_TILE < NUM_COL_TILES-1) {
            tc_flush_tiles<T, TILE_WIDTH, NUM_COL_TILES, NUM_ROW_TILES,CUR_ROW_TILE, CUR_COL_TILE+1>(dest);
        } else {
            tc_flush_tiles<T, TILE_WIDTH, NUM_COL_TILES, NUM_ROW_TILES,CUR_ROW_TILE+1, 0>(dest);
        }
    }
}
